...
async def submit_train_job(
    ...
    response = requests.post(
        f"{TRAINER_SERVER}/submit/train?{url}",
        files={"pdb_file": pdb_file.file, "gpf_file": gpf_file.file},
    )
...
async def submit_eval_job(
    ...
    params = {"job_id": job_id, "user_id": user.id, "num_molecules": num_molecules}
...
async def login(form_data: OAuth2PasswordRequestForm = Depends()):
    user: Union[bool, UserInDB] = await user_manager.authenticate_user(
    ...
    access_token: str = await user_manager.create_access_token(data={"sub": user.email})
