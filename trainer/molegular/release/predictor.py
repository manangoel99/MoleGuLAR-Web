...
from molegular.release.utils import (
    get_fp,
    get_desc,
    normalize_desc,
...
class VanillaQSAR(object):
    def __init__(
        ...
        if model_type not in ["classifier", "regressor"]:
            raise InvalidArgumentError(
                "model type must be either" "classifier or regressor"
            )
    ...
    def fit_model(self, data, cv_split="stratified"):
        ...
        if self.model_type == "classifier" and data.binary_y is not None:
            ...
            y = data.y
        cross_val_data, cross_val_labels = cross_validation_split(
            x=x, y=y, split=cv_split, n_folds=self.ensemble_size
        )
        for i in range(self.ensemble_size):
    ...
    def predict(self, objects=None, average=True, get_features=None, **kwargs):
        objects = np.array(objects)
        invalid_objects = []
        processed_objects = []
        if get_features is not None:
            x, processed_indices, invalid_indices = get_features(objects, **kwargs)
